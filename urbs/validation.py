...
def validate_input(data):
    ...
    for stf, sit, pro in data["process"].index:
        for com in data["commodity"].index.get_level_values("Commodity"):
            simplified_pro_com_index = [
                (st, p, c) for st, p, c, d in data["process_commodity"].index.tolist()
            ]
            simplified_com_index = [
                (st, s, c) for st, s, c, t in data["commodity"].index.tolist()
            ]
            if (stf, pro, com) in simplified_pro_com_index and (
                stf,
                sit,
                com,
            ) not in simplified_com_index:
                raise ValueError(
                    "Commodities used in a process at a site must"
    ...
    for key in data:
        if not data[key].index[data[key].index.duplicated()].unique().empty:
            if key == "global_prop":
                raise ValueError(
                    'Some support time frames are duplicated in sheet "Global"'
                )
            else:
                raise ValueError(
                    "The following indices are duplicated "
                    + str(data[key].index[data[key].index.duplicated()].unique())
                )
    ...
    for index in data["process"].index:
        if not (
            data["process"].loc[index]["cap-lo"] <= data["process"].loc[index]["cap-up"]
            and data["process"].fillna(0).loc[index]["inst-cap"]
            <= data["process"].loc[index]["cap-up"]
        ):
            raise ValueError(
                "Ensure cap_lo <= cap_up and inst_cap <= cap_up for all processes."
            )

    if not data["transmission"].empty:
        for index in data["transmission"].index:
            if not (
                data["transmission"].loc[index]["cap-lo"]
                <= data["transmission"].loc[index]["cap-up"]
                and data["transmission"].fillna(0).loc[index]["inst-cap"]
                <= data["transmission"].loc[index]["cap-up"]
            ):
                raise ValueError(
                    "Ensure cap_lo <= cap_up and"
        ...
        if "reactance" in data["transmission"].keys():
            for index in data["transmission"].index:
                if data["transmission"].loc[index]["reactance"] < 0:
                    raise ValueError(
                        "Ensure for DCPF transmission lines: reactance > 0 "
                    )
                if data["transmission"].loc[index]["reactance"] > 0:
                    if data["transmission"].loc[index]["eff"] != 1:
                        raise ValueError(
                            "Ensure efficiency of DCPF Transmission Lines are 1"
                        )
                    if not data["transmission"].loc[index]["base_voltage"] > 0:
                        raise ValueError(
                            "Ensure base voltage of DCPF transmission lines are "
                            "greater than 0"
                        )
                    if not (0 < data["transmission"].loc[index]["difflimit"] <= 90):
                        raise ValueError(
                            "Ensure angle difference of DCPF transmission lines "
    ...
    if not data["storage"].empty:
        for index in data["storage"].index:
            if not (
                data["storage"].loc[index]["cap-lo-p"]
                <= data["storage"].loc[index]["cap-up-p"]
                and data["storage"].fillna(0).loc[index]["inst-cap-p"]
                <= data["storage"].loc[index]["cap-up-p"]
            ):
                raise ValueError(
                    "Ensure cap_lo <= cap_up and"
                ...
                and data["storage"].fillna(0).loc[index]["inst-cap-c"]
                <= data["storage"].loc[index]["cap-up-c"]
            ):
                raise ValueError(
                    "Ensure cap_lo <= cap_up and inst_cap <= "
    ...
    if not data["storage"].empty:
        if (data["storage"]["init"] > 1).any():
            raise ValueError(
                "In worksheet 'storage' all values in column "
    ...
    if "maxperstep" in list(data["commodity"]):
        raise KeyError(
            "Maximum allowable commodities are defined by per "
    ...
    for site in data["commodity"].index.levels[1].tolist():
        if site not in data["site"].index.levels[1].tolist():
            raise KeyError(
                "All names in the column 'Site' in input worksheet "
    ...
    if not data["storage"].empty:
        for site in data["storage"].index.levels[1].tolist():
            if site not in data["site"].index.levels[1].tolist():
                raise KeyError(
                    "All names in the column 'Site' in input "
...
def validate_dc_objective(data, objective):
    if not data["transmission"].empty:
        if "reactance" in data["transmission"].keys():
            if (
                any(data["transmission"]["reactance"] > 0)
                and (objective == "CO2")
