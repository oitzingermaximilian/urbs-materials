...
def add_dsm(m):
    ...
    # Variables
    m.dsm_up = pyomo.Var(
        m.tm, m.dsm_site_tuples, within=pyomo.NonNegativeReals, doc="DSM upshift"
    )
    m.dsm_down = pyomo.Var(
        m.dsm_down_tuples, within=pyomo.NonNegativeReals, doc="DSM downshift"
    )
...
def def_dsm_variables_rule(m, tm, stf, sit, com):
    dsm_down_sum = 0
    for tt in dsm_time_tuples(
        tm,
        m.timesteps[1:],
        max(int(1 / m.dt * m.dsm_dict["delay"][(stf, sit, com)]), 1),
    ):
        dsm_down_sum += m.dsm_down[tm, tt, stf, sit, com]
    return dsm_down_sum == (
        m.dsm_up[tm, stf, sit, com] * m.dsm_dict["eff"][(stf, sit, com)]
    )
...
def res_dsm_upward_rule(m, tm, stf, sit, com):
    return m.dsm_up[tm, stf, sit, com] <= (
        m.dt * m.dsm_dict["cap-max-up"][(stf, sit, com)]
    )
...
def res_dsm_maximum_rule(m, tm, stf, sit, com):
    ...
    max_dsm_limit = m.dt * max(
        m.dsm_dict["cap-max-up"][(stf, sit, com)],
...
def res_dsm_recovery_rule(m, tm, stf, sit, com):
    dsm_up_sum = 0
    for t in dsm_recovery(
        tm,
        m.timesteps[1:],
        max(int(1 / m.dt * m.dsm_dict["recov"][(stf, sit, com)]), 1),
    ):
        dsm_up_sum += m.dsm_up[t, stf, sit, com]
    return dsm_up_sum <= (
        m.dsm_dict["cap-max-up"][(stf, sit, com)] * m.dsm_dict["delay"][(stf, sit, com)]
    )
...
def dsm_surplus(m, tm, stf, sit, com):
    ...
    if (stf, sit, com) in m.dsm_site_tuples:
        return -m.dsm_up[tm, stf, sit, com] + sum(
            m.dsm_down[t, tm, stf, sit, com]
            for t in dsm_time_tuples(
                tm,
                m.timesteps[1:],
                max(int(1 / m.dt * m.dsm_dict["delay"][(stf, sit, com)]), 1),
...
def dsm_down_time_tuples(time, sit_com_tuple, m):
    ...
    for stf, site, commodity in sit_com_tuple:
        for step1 in time:
            for step2 in range(
                step1 - max(int(delay[stf, site, commodity] / m.dt.value), 1),
                step1 + max(int(delay[stf, site, commodity] / m.dt.value), 1) + 1,
            ):
                if lb <= step2 <= ub:
