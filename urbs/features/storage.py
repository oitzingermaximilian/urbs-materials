...
def add_storage(m):
    ...
    if m.mode["int"]:
        m.operational_sto_tuples = pyomo.Set(
            ...
            initialize=[
                ...
                for (sit, sto, com, stf, stf_later) in op_sto_tuples(m.sto_tuples, m)
            ],
        ...
        m.inst_sto_tuples = pyomo.Set(
            ...
            initialize=[
                (sit, sto, com, stf) for (sit, sto, com, stf) in inst_sto_tuples(m)
            ],
    ...
    m.cap_sto_c_new = pyomo.Var(
        m.sto_tuples, within=pyomo.NonNegativeReals, doc="New storage size (MWh)"
    )
    m.cap_sto_p_new = pyomo.Var(
        m.sto_tuples, within=pyomo.NonNegativeReals, doc="New  storage power (MW)"
    )

    # storage capacities as expression objects
    m.cap_sto_c = pyomo.Expression(
        m.sto_tuples, rule=def_storage_capacity_rule, doc="Total storage size (MWh)"
    )
    m.cap_sto_p = pyomo.Expression(
        m.sto_tuples, rule=def_storage_power_rule, doc="Total storage power (MW)"
    )
...
def def_storage_state_rule(m, t, stf, sit, sto, com):
    return (
        ...
        * (1 - m.storage_dict["discharge"][(stf, sit, sto, com)]) ** m.dt.value
    ...
    if m.mode["int"]:
        if (sit, sto, com, stf) in m.inst_sto_tuples:
            if (min(m.stf), sit, sto, com) in m.sto_const_cap_c_dict:
                ...
                cap_sto_c = (
                    sum(
                        m.cap_sto_c_new[stf_built, sit, sto, com]
    ...
    if m.mode["int"]:
        if (sit, sto, com, stf) in m.inst_sto_tuples:
            if (min(m.stf), sit, sto, com) in m.sto_const_cap_p_dict:
                cap_sto_p = m.storage_dict["inst-cap-p"][(min(m.stf), sit, sto, com)]
            else:
                cap_sto_p = (
                    sum(
                        m.cap_sto_p_new[stf_built, sit, sto, com]
                        for stf_built in m.stf
                        if (sit, sto, com, stf_built, stf) in m.operational_sto_tuples
                    )
...
def res_storage_input_by_power_rule(m, t, stf, sit, sto, com):
    return m.e_sto_in[t, stf, sit, sto, com] <= m.dt * m.cap_sto_p[stf, sit, sto, com]


# storage output <= storage power
def res_storage_output_by_power_rule(m, t, stf, sit, sto, co):
    return m.e_sto_out[t, stf, sit, sto, co] <= m.dt * m.cap_sto_p[stf, sit, sto, co]


# storage content <= storage capacity
def res_storage_state_by_capacity_rule(m, t, stf, sit, sto, com):
    return m.e_sto_con[t, stf, sit, sto, com] <= m.cap_sto_c[stf, sit, sto, com]
...
def def_initial_storage_state_rule(m, stf, sit, sto, com):
    return (
        m.e_sto_con[m.t.at(1), stf, sit, sto, com]
        == m.cap_sto_c[stf, sit, sto, com]
...
def storage_balance(m, tm, stf, sit, com):
    ...
    return sum(
        m.e_sto_in[(tm, stframe, site, storage, com)]
...
def storage_cost(m, cost_type):
    ...
    if cost_type == "Invest":
        ...
        return sum(
            (
                m.cap_sto_p[s] * m.storage_dict["fix-cost-p"][s]
...
def specific_storage_cost(m, stf, sit, sto, com, cost_type):
    ...
    if cost_type == "Invest":
        ...
        cost_spec_storage = (
            m.cap_sto_p[stf, sit, sto, com]
            * m.storage_dict["fix-cost-p"][stf, sit, sto, com]
            + m.cap_sto_c[stf, sit, sto, com]
            * m.storage_dict["fix-cost-c"][stf, sit, sto, com]
        ) * m.storage_dict["cost_factor"][stf, sit, sto, com]
        return m.storage_costs[stf, sit, sto, com, cost_type] == cost_spec_storage
    elif cost_type == "Variable":
        cost_spec_storage = sum(
            m.e_sto_con[tm, stf, sit, sto, com]
            * m.weight
            * m.storage_dict["var-cost-c"][stf, sit, sto, com]
            * m.storage_dict["cost_factor"][stf, sit, sto, com]
            + (m.e_sto_in[tm, stf, sit, sto, com] + m.e_sto_out[tm, stf, sit, sto, com])
            * m.weight
            * m.storage_dict["var-cost-p"][stf, sit, sto, com]
            * m.storage_dict["cost_factor"][stf, sit, sto, com]
            for tm in m.tm
        )
        return m.storage_costs[stf, sit, sto, com, cost_type] == cost_spec_storage
