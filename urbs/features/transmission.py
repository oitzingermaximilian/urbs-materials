...
def remove_duplicate_transmission(transmission_keys):
    ...
    while i < len(tra_tuple_list):
        for k in range(len(tra_tuple_list)):
            if (
                tra_tuple_list[i][1] == tra_tuple_list[k][2]
                and tra_tuple_list[i][2] == tra_tuple_list[k][1]
                and tra_tuple_list[i][0] == tra_tuple_list[k][0]
                and tra_tuple_list[i][3] == tra_tuple_list[k][3]
            ):
...
def add_transmission(m):
    ...
    m.tra = pyomo.Set(initialize=indexlist, doc="Set of transmission technologies")

    # transmission tuples
    m.tra_tuples = pyomo.Set(
        within=m.stf * m.sit * m.sit * m.tra * m.com,
        initialize=tuple(m.transmission_dict["eff"].keys()),
        doc="Combinations of possible transmissions, e.g. (2020,South,Mid,hvac,Elec)",
    )
    ...
    if m.mode["int"]:
        m.operational_tra_tuples = pyomo.Set(
            ...
            initialize=[
                (sit, sit_, tra, com, stf, stf_later)
                for (sit, sit_, tra, com, stf, stf_later) in op_tra_tuples(
                    m.tra_tuples, m
                )
            ],
        ...
        m.inst_tra_tuples = pyomo.Set(
            ...
            initialize=[
                (sit, sit_, tra, com, stf)
                for (sit, sit_, tra, com, stf) in inst_tra_tuples(m)
            ],
            doc="Installed transmissions that are still operationalthrough stf",
        )
    ...
    m.res_transmission_capacity = pyomo.Constraint(
        ...
        doc="transmission.cap-lo <= total transmission capacity <= transmission.cap-up",
    )
...
def add_transmission_dc(m):
    ...
    m.voltage_angle = pyomo.Var(
        m.tm, m.stf, m.sit, within=pyomo.Reals, doc="Voltage angle of a site"
    )
    ...
    if m.mode["int"]:
        if (sin, sout, tra, com, stf) in m.inst_tra_tuples:
            if (min(m.stf), sin, sout, tra, com) in m.tra_const_cap_dict:
                ...
                cap_tra = (
                    sum(
                        m.cap_tra_new[stf_built, sin, sout, tra, com]
                        for stf_built in m.stf
                        if (sin, sout, tra, com, stf_built, stf)
                        in m.operational_tra_tuples
                    )
        ...
        if (stf, sin, sout, tra, com) in m.tra_const_cap_dict:
            cap_tra = m.transmission_dict["inst-cap"][(stf, sin, sout, tra, com)]
        else:
            cap_tra = (
...
def def_transmission_output_rule(m, tm, stf, sin, sout, tra, com):
    return (
        m.e_tra_out[tm, stf, sin, sout, tra, com]
...
def transmission_balance(m, tm, stf, sit, com):
    ...
    return sum(
        m.e_tra_in[(tm, stframe, site_in, site_out, transmission, com)]
        # exports increase balance
        for stframe, site_in, site_out, transmission, commodity in m.tra_tuples
        if (site_in == sit and stframe == stf and commodity == com)
    ) - sum(
        m.e_tra_out[(tm, stframe, site_in, site_out, transmission, com)]
...
def specific_transmission_cost(m, stf, sit, sit_, tra, com, cost_type):
    ...
    if cost_type == "Invest":
        cost_spec_transmission = (
            m.cap_tra_new[stf, sit, sit_, tra, com]
        ...
        return (
            m.transmission_costs[stf, sit, sit_, tra, com, cost_type]
            == cost_spec_transmission
        )
    elif cost_type == "Fixed":
        cost_spec_transmission = (
            m.cap_tra[stf, sit, sit_, tra, com]
            * m.transmission_dict["fix-cost"][stf, sit, sit_, tra, com]
        ...
        if m.mode["dpf"]:
            cost_spec_transmission = sum(
                m.e_tra_in[tm, stf, sit, sit_, tra, com]
                * m.weight
                * m.transmission_dict["var-cost"][stf, sit, sit_, tra, com]
                * m.transmission_dict["cost_factor"][stf, sit, sit_, tra, com]
                for tm in m.tm
            ) + (
                m.e_tra_abs[tm, stf, sit, sit_, tra, com]
                * m.weight
                * m.transmission_dict["var-cost"][stf, sit, sit_, tra, com]
                * m.transmission_dict["cost_factor"][stf, sit, sit_, tra, com]
                for tm in m.tm
        ...
        for stf_later in sorted_stf:
            ...
            if stf_later == max(sorted_stf):
                if (
                    ...
                    - 1
                    <= stf
                    + m.transmission_dict["depreciation"][(stf, sit1, sit2, tra, com)]
                ):
                    op_tra.append((sit1, sit2, tra, com, stf, stf_later))
            elif (
                sorted_stf[index_helper + 1]
                <= stf
                + m.transmission_dict["depreciation"][(stf, sit1, sit2, tra, com)]
                and stf <= stf_later
            ):
