...
class cost_scrap_rule(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech):
        ...
        magnet_intensity = sum(m.scrap_content[tech, mat] for mat in magnet_mats if (tech, mat) in m.scrap_content)
        ...
        bulk_weight = sum(m.scrap_content[tech, mat] for mat in m.materials if mat not in magnet_mats and (tech, mat) in m.scrap_content)
        ...
        if tech in ["windon", "windoff"]:
            ...
            magnet_route_opex = (m.f_scrap_rec_magnet[stf, location, tech] * magnet_weight) + (m.f_scrap_rec_bulk[stf, location, tech] * bulk_weight)
            bulk_route_opex = m.f_scrap_rec_bulk[stf, location, tech] * bulk_weight
            
            opex_cost = (magnet_route_opex * m.capacity_scrap_magnet_route[stf, location, tech] +
                         bulk_route_opex * m.capacity_scrap_bulk_route[stf, location, tech])
                         
            magnet_route_capex = (m.f_scrap_capex_magnet[stf, location, tech] * magnet_weight) + (m.f_scrap_capex_bulk[stf, location, tech] * bulk_weight)
            bulk_route_capex = m.f_scrap_capex_bulk[stf, location, tech] * bulk_weight
            
            capex_cost = ((m.scraphandling_cap_new_magnet[stf, location, tech] * magnet_route_capex * (f_inv - f_over)) +
                          (m.scraphandling_cap_new_bulk[stf, location, tech] * bulk_route_capex * (f_inv - f_over)))
                          
            magnet_route_fom = (m.f_scrap_fom_magnet[stf, location, tech] * magnet_weight) + (m.f_scrap_fom_bulk[stf, location, tech] * bulk_weight)
            bulk_route_fom = m.f_scrap_fom_bulk[stf, location, tech] * bulk_weight
            
            fom_cost = (magnet_route_fom * m.capacity_scrap_handling_magnet_total[stf, location, tech] +
                        bulk_route_fom * m.capacity_scrap_handling_bulk_total[stf, location, tech])
            ...
            if hasattr(m, 'PRICEREDUCTION_SCRAP_WINDON_CAP_BASED'):
                if tech == "windon":
                    savings = m.PRICEREDUCTION_SCRAP_WINDON_CAP_BASED[stf, location] * magnet_weight
                    savings_capex = m.PRICEREDUCTION_SCRAP_WINDON_CAPEX_CAP_BASED[stf, location] * magnet_weight * (f_inv - f_over)
                    savings_fom = m.PRICEREDUCTION_SCRAP_WINDON_FOM_CAP_BASED[stf, location] * magnet_weight
                elif tech == "windoff":
                    savings = m.PRICEREDUCTION_SCRAP_WINDOFF_CAP_BASED[stf, location] * magnet_weight
                    savings_capex = m.PRICEREDUCTION_SCRAP_WINDOFF_CAPEX_CAP_BASED[stf, location] * magnet_weight * (f_inv - f_over)
                    savings_fom = m.PRICEREDUCTION_SCRAP_WINDOFF_FOM_CAP_BASED[stf, location] * magnet_weight
            ...
            opex_cost = m.f_scrap_rec[stf, location, tech] * total_weight * m.capacity_scrap_rec[stf, location, tech]
            capex_cost = m.scraphandling_cap_new[stf, location, tech] * m.f_scrap_capex[stf, location, tech] * total_weight * (f_inv - f_over)
            fom_cost = m.f_scrap_fom[stf, location, tech] * total_weight * m.capacity_scrap_handling_total[stf, location, tech]
            ...
            if tech == "solarPV" and hasattr(m, 'PRICEREDUCTION_SCRAP_SOLAR_CAP_BASED'):
                savings = m.PRICEREDUCTION_SCRAP_SOLAR_CAP_BASED[stf, location] * total_weight
                savings_capex = m.PRICEREDUCTION_SCRAP_SOLAR_CAPEX_CAP_BASED[stf, location] * total_weight * (f_inv - f_over)
                savings_fom = m.PRICEREDUCTION_SCRAP_SOLAR_FOM_CAP_BASED[stf, location] * total_weight

        # 3. Final Equation
        return m.cost_scrap[stf, location, tech] == base_cost - savings - savings_capex - savings_fom
...
class capacity_scrap_routing_sum_rule(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech):
        if tech in ["windon", "windoff"]:
            return m.capacity_scrap_rec[stf, location, tech] == m.capacity_scrap_magnet_route[stf, location, tech] + m.capacity_scrap_bulk_route[stf, location, tech]
        else:
            return pyomo.Constraint.Skip
...
class ScrapHandlingOutputDecreaseLimitRule(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech):
        ...
        if stf == min(m.stf):
            ...
            lhs = (m.capacity_scrap_rec[stf - 1, location, tech] -
                   m.capacity_scrap_rec[stf, location, tech])
...
class ScrapHandlingOutputGrowthLimitRule(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech):
        ...
        if stf == min(m.stf):
            ...
            lhs = (m.capacity_scrap_rec[stf, location, tech] - m.capacity_scrap_rec[stf - 1, location, tech])
...
class ScrapHandlingCapacitiesOutputLimitMagnetRule(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech):
        if tech in ["windon", "windoff"]:
            return m.capacity_scrap_handling_magnet_total[stf, location, tech] >= m.capacity_scrap_magnet_route[stf, location, tech]
        else:
...
class ScrapHandlingCapacitiesOutputLimitBulkRule(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech):
        if tech in ["windon", "windoff"]:
            return m.capacity_scrap_handling_bulk_total[stf, location, tech] >= m.capacity_scrap_bulk_route[stf, location, tech]
        else:
...
class ScrapHandlingCapacitiesSizeMagnetRule(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech):
        if tech in ["windon", "windoff"]:
            ...
            rhs = sum(m.scraphandling_cap_new_magnet[y, location, tech] for y in m.stf if y <= stf)
            return lhs == rhs
        else:
...
class ScrapHandlingCapacitiesSizeBulkRule(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech):
        if tech in ["windon", "windoff"]:
            ...
            rhs = sum(m.scraphandling_cap_new_bulk[y, location, tech] for y in m.stf if y <= stf)
            return lhs == rhs
        else:
    ...
    def apply_rule(self, m, stf, location, tech):
        if tech in ["windon", "windoff"]:
            if stf == min(m.stf):
                ...
                lhs = (m.capacity_scrap_magnet_route[stf, location, tech] -
                       m.capacity_scrap_magnet_route[stf - 1, location, tech])
                rhs = (200.0 + 0.50 * m.capacity_scrap_magnet_route[stf - 1, location, tech])
                return lhs <= rhs
        else:
            return pyomo.Constraint.Skip
    ...
    def apply_rule(self, m, stf, location, tech):
        if tech in ["windon", "windoff"]:
            if stf == min(m.stf):
                ...
                lhs = (m.capacity_scrap_bulk_route[stf, location, tech] -
                       m.capacity_scrap_bulk_route[stf - 1, location, tech])
                rhs = (200.0 + 0.50 * m.capacity_scrap_bulk_route[stf - 1, location, tech])
                return lhs <= rhs
        else:
            return pyomo.Constraint.Skip
...
class ScrapHandlingOutputDecreaseLimitMagnetRule(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech):
        if tech in ["windon", "windoff"]:
            if stf == min(m.stf):
                return pyomo.Constraint.Skip
            lhs = (m.capacity_scrap_magnet_route[stf - 1, location, tech] -
...
class ScrapHandlingOutputDecreaseLimitBulkRule(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech):
        if tech in ["windon", "windoff"]:
            if stf == min(m.stf):
                return pyomo.Constraint.Skip
            lhs = (m.capacity_scrap_bulk_route[stf - 1, location, tech] -
...
def apply_scrap_constraints(m):
    m.decommissioned_capacity_rule = pyomo.Constraint(
        ...
        rule=lambda m, stf, loc, tech: decommissioned_capacity_rule().apply_rule(m, stf, loc, tech),
    )
    m.capacity_scrap_dec_rule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: capacity_scrap_dec_rule().apply_rule(m, stf, loc, tech),
    )
    m.capacity_scrap_total_rule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: capacity_scrap_total_rule().apply_rule(m, stf, loc, tech),
    )
    ...
    m.capacity_scrap_routing_magnet_rule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: capacity_scrap_routing_magnet_rule().apply_rule(m, stf, loc, tech),
    )
    m.capacity_scrap_routing_bulk_rule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: capacity_scrap_routing_bulk_rule().apply_rule(m, stf, loc, tech),
    )
    m.capacity_scrap_routing_sum_rule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: capacity_scrap_routing_sum_rule().apply_rule(m, stf, loc, tech),
    )
    
    # Facility constraints
    m.ScrapHandlingCapacitiesOutputLimitRule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: ScrapHandlingCapacitiesOutputLimitRule().apply_rule(m, stf, loc, tech),
    )
    m.ScrapHandlingOutputDecreaseLimitRule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: ScrapHandlingOutputDecreaseLimitRule().apply_rule(m, stf, loc, tech),
    )
    m.ScrapHandlingCapacitiesSizeRule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: ScrapHandlingCapacitiesSizeRule().apply_rule(m, stf, loc, tech),
    )
    m.ScrapHandlingOutputGrowthLimitRule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: ScrapHandlingOutputGrowthLimitRule().apply_rule(m, stf, loc, tech),
    )
    m.ScrapHandlingCapacitiesOutputLimitMagnetRule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: ScrapHandlingCapacitiesOutputLimitMagnetRule().apply_rule(m, stf, loc, tech),
    )
    m.ScrapHandlingCapacitiesOutputLimitBulkRule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: ScrapHandlingCapacitiesOutputLimitBulkRule().apply_rule(m, stf, loc, tech),
    )
    m.ScrapHandlingCapacitiesSizeMagnetRule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: ScrapHandlingCapacitiesSizeMagnetRule().apply_rule(m, stf, loc, tech),
    )
    m.ScrapHandlingCapacitiesSizeBulkRule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: ScrapHandlingCapacitiesSizeBulkRule().apply_rule(m, stf, loc, tech),
    )
    m.ScrapHandlingOutputGrowthLimitMagnetRule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: ScrapHandlingOutputGrowthLimitMagnetRule().apply_rule(m, stf, loc, tech),
    )
    m.ScrapHandlingOutputGrowthLimitBulkRule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: ScrapHandlingOutputGrowthLimitBulkRule().apply_rule(m, stf, loc, tech),
    )
    m.ScrapHandlingOutputDecreaseLimitMagnetRule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: ScrapHandlingOutputDecreaseLimitMagnetRule().apply_rule(m, stf, loc, tech),
    )
    m.ScrapHandlingOutputDecreaseLimitBulkRule = pyomo.Constraint(
        m.stf, m.location, m.tech,
        rule=lambda m, stf, loc, tech: ScrapHandlingOutputDecreaseLimitBulkRule().apply_rule(m, stf, loc, tech),
    )