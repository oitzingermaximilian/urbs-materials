...
class nzia_flex_rule(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech, target_techs):
        ...
        total_supply = sum(m.Supply[stf, location, tech, stage] for stage in m.stages)
...
class eu_extraction_constraint(AbstractConstraint):
    def apply_rule(self, m, stf, mat):
        ...
        if isinstance(availability, pyomo.Param) or isinstance(
...
def apply_scenario_constraints(
    ...
    m.eu_recycling_constraint = pyomo.Constraint(
        m.stf, m.materials, rule=lambda m, y, mat: recycling_logic.apply_rule(m, y, mat)
    )
    ...
    if not crma_active:
        ...
        m.eu_crma_combined_constraint.deactivate()
        print(
            "✅ CRMA SEPARATED: Active (>=2030) - Strict 10% Mining & 25% Recycling enforced separately."
        )
        ...
        print(
            f"✅ CRMA COMBINED:  Active (>=2030) - Joint (Mining + Recycling) >= {int(crma_quota * 100)}% resilience pool."
        )
