            ...
            for location in m.location:
                for tech in m.tech:
                    ...
                    if cost_type_new in [
                        ...
                        total_cost += base * invcost_factor(
                            lifetime, stf
                        ...
                        if cost_type_new == "O_and_M":
                            base = (
                                m.O_and_M_costs[stf, location, tech]
                                * m.capacity_ext[stf, location, tech]
                            )
                        elif cost_type_new == "Storagecost":
                            base = (
                                m.STORAGECOST[location, tech]
                                * m.capacity_ext_stock[stf, location, tech]
                            )
...
class CalculateYearlyImportCost(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech):
        ...
        expr = m.costs_ext_import[stf, location, tech] == base * invcost_factor(
            lifetime, stf
        ) - base * overpay_factor(lifetime, stf)
        return expr
...
class CalculateYearlyEUPrimary(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech):
        ...
        expr = m.costs_EU_primary[stf, location, tech] == base * invcost_factor(
            lifetime, stf
        ) - base * overpay_factor(lifetime, stf)
        return expr
...
class CalculateYearlyEUSecondary(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech):
        ...
        expr = m.costs_EU_secondary[stf, location, tech] == base * invcost_factor(
            lifetime, stf
        ) - base * overpay_factor(lifetime, stf)
        return expr
