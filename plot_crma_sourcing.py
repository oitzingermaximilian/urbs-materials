...
OUTPUT_DIR = os.path.join("plot_iew", "minerals_sourcing")

MATERIAL_GROUPS = {
    "Solar PV Only": ["silicon"],
    "Crossover (Solar & Wind)": ["aluminum", "copper"],
    "Wind Only": ["boron", "cobalt", "dysprosium", "gallium", "graphite", "lithium", "manganese", "neodymium", "nickel", "niobium", "praseodymium", "terbium", "titanium", "vanadium"]
}
...
def catmull_rom_spline_1d(y, n_interp=50):
    ...
    for i in range(N):
        ...
        for t in np.linspace(0, 1, n_interp, endpoint=False):
            val = 0.5 * (2*p1 + (-p0 + p2)*t + (2*p0 - 5*p1 + 4*p2 - p3)*(t**2) + (-p0 + 3*p1 - 3*p2 + p3)*(t**3))
            y_smooth.append(val)
...
def create_clustered_bar_charts(scenario_data, output_dir):
    base_patches = [mpatches.Patch(facecolor=fc, edgecolor="black", label=lab) for fc, lab in zip(bar_colors, components)]
    imports_patch = mpatches.Patch(facecolor=imports_color, edgecolor="black", label="Imports")
    ...
    for mat in CRMA_TARGET_MATERIALS:
        mat_has_demand = any(scenario_data[q][mat]["demand"].sum() > 1e-6 for q in scenario_data)
        ...
        offset_trans_rel = mtransforms.ScaledTranslation(0, 20 / 72, fig.dpi_scale_trans)
        text_trans_rel = ax.transData + offset_trans_rel
        
        for i, win_name in enumerate(windows):
            years = BINS[win_name]
            start_offset = -(max_clusters * width + (max_clusters - 1) * gap) / 2 + width / 2
            
            for j, q in enumerate(QUOTAS):
                x_pos = x_base[i] + start_offset + j * (width + gap)
                
                if q not in scenario_data: continue
                ...
                ax.bar(x_pos, 1, width=width, facecolor=imports_color, edgecolor="black", linewidth=0.8, zorder=0)
                
                bottom = 0
                for comp, color in zip(["mined", "recycled"], bar_colors):
                    val = mined if comp == "mined" else recycled
                    frac = val / denom
                    ax.bar(x_pos, frac, width=width, bottom=bottom, facecolor=color, edgecolor="black", linewidth=0.8, zorder=1)
                    bottom += frac
                    
                font_size = max(6, min(16, 80 / max_clusters))
                ax.text(x_pos, 0.95, f"{q}%", ha="center", va="bottom", rotation=90, fontsize=font_size, transform=text_trans_rel)
        ...
        ax.legend(handles=legend_handles, fontsize=FS_LEGEND_1ROW, frameon=True, loc="lower center", bbox_to_anchor=(0.5, LEGEND_Y_POS), ncol=3, borderaxespad=0)
        ...
        group_dir = os.path.join(output_dir, mat_group.replace(" & ", "_").replace(" ", "_"))
...
def create_radar_grid(scenario_data, output_dir):
    ...
    for idx, mat in enumerate(active_mats):
        ...
        for q in QUOTAS:
            ...
            ax.plot(theta_smooth, values_smooth, color=RADAR_COLORS[q], linewidth=2.5, alpha=0.9, label=f"S{q}")
    ...
    handles = [Line2D([0], [0], color=RADAR_COLORS[q], lw=2.5, label=f"S{q}") for q in QUOTAS]
    fig.legend(handles=handles, loc='lower center', ncol=len(QUOTAS), fontsize=14, frameon=False, bbox_to_anchor=(0.5, 0.02))
...
def create_individual_radars(scenario_data, output_dir):
    ...
    for mat in CRMA_TARGET_MATERIALS:
        ...
        if mat.lower() == "aluminum":
            ax.set_yticklabels(["25%", "50%", "75%", "100%"], color="#555555", size=11, weight="bold")
    ...
    handles = [Line2D([0], [0], color=RADAR_COLORS[q], lw=3, label=f"S{q}") for q in QUOTAS]
    fig_leg.legend(handles=handles, loc='center', ncol=len(QUOTAS), fontsize=14, frameon=True)
...
def create_selected_examples_grid(scenario_data, output_dir):
    print(f"📊 Building Selected Examples Grid (Aluminum, Copper, Neodymium, Silicon)...")
    ...
    for idx, mat in enumerate(mats_to_plot):
        ...
        if mat.lower() == "aluminum":
            ax.set_yticklabels(["25%", "50%", "75%", "100%"], color="black", size=11, weight="bold")
        else:
            ax.set_yticklabels([])
    ...
    handles = [Line2D([0], [0], color=RADAR_COLORS[q], lw=3, label=f"S{q}") for q in QUOTAS]
    leg = fig.legend(handles=handles, loc='lower center', ncol=len(QUOTAS), fontsize=15, frameon=True, bbox_to_anchor=(0.5, 0.01))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Plot CRMA sourcing radars and bar charts")
    parser.add_argument('--only-examples', action='store_true', help="Only plot the 2x2 Selected Examples Grid")
