...
def create_vertical_marginal_plot(tech, scenario_data, stages, output_dir=OUTPUT_DIR):
        ...
        for j, stage in enumerate(stages):
            ...
            for k, scen_key in enumerate(STEP_ORDER):
                ...
                current_val = (
                    ...
                    if (
                        df_scen is not None
                        and stage in df_scen.columns
                        and year in df_scen.index
                    )
                    else 0
                )
                ...
                if scen_key == "Base_case":
                    ...
                    init_val = (
                        df_scen.loc[BASELINE_YEAR, stage]
                        if (
                            df_scen is not None
                            and stage in df_scen.columns
                            and BASELINE_YEAR in df_scen.index
                        )
                        else 0
                    )
                    if init_val > 0:
                    ...
                    prev_val = (
                        ...
                        if (
                            df_prev is not None
                            and stage in df_prev.columns
                            and year in df_prev.index
                        )
                        else 0
                    )
    ...
    handles.append(
        mpatches.Patch(
            facecolor=COLORS["Base_case"], edgecolor="black", label="Base Case"
        )
    )
    handles.append(
        Line2D([0], [0], color="yellow", linewidth=2.5, label="Initial Cap. (2024)")
    )
    for q in QUOTAS:
        c = COLORS[q]
        handles.append(
            mpatches.Patch(
                facecolor="white", hatch="////", edgecolor=c, label=f"CRMA {q}%"
            )
        )
...
def create_horizontal_absolute_plot(tech, scenario_data, stages, output_dir=OUTPUT_DIR):
    ...
    yticklabels = [
        f"{stage.replace('On', '').replace('Off', '')} {year}"
        for stage, year in categories
    ]
    ...
    for q in QUOTAS:
        handles.append(
            mpatches.Patch(facecolor=COLORS[q], edgecolor="black", label=f"CRMA {q}%")
        )
...
def create_diverging_delta_plot(tech, scenario_data, stages, output_dir=OUTPUT_DIR):
    ...
    fig, axs = plt.subplots(
        n_rows, n_cols, figsize=(3 * n_cols, 2.5 * n_rows), sharex="row", sharey=True
    )
    ...
    for i, stage in enumerate(stages):
        for j, year in enumerate(YEARS_TO_PLOT):
            ...
            if (
                df_base is not None
                and stage in df_base.columns
                and year in df_base.index
            ):
            ...
            if j == 0:
                ...
                ax.set_yticks(y_pos)
                ax.set_yticklabels(
                    [f"S{q}" for q in QUOTAS], fontsize=14, weight="bold"
                )
...
def main():
    ...
    for tech, stages in TECH_STAGE_MAP.items():
        ...
        if base_file:
            tech_data["Base_case"] = load_clean_data(
                base_file, tech_filter=tech, stages=stages
            )
