...
TECHS = {"solarPV": "Solar PV", "windon": "Onshore Wind", "windoff": "Offshore Wind"}
...
def create_boxplot(tech_id, tech_label, scenarios_dict, output_dir):
    ...
    ax.set_ylabel(
        "Total Installed Capacity (GW)", fontsize=18, labelpad=15, weight="bold"
    )
...
def plot_combined_heatmap(all_gradients, output_dir):
    ...
    deg_cols = [
        ...
        if f"Degrees_{interval}" in df.columns
    ]
    ...
    df["Technology"] = pd.Categorical(
        df["Technology"], categories=tech_order, ordered=True
    )
    ...
    ax.set_xticks(np.arange(-0.5, len(display_cols), 1), minor=True)
    ...
    ax.set_title(
        "Capacity Gradient by Scenario & Technology (Degrees)",
        fontsize=16,
        weight="bold",
        pad=15,
    )
...
def create_combined_relative_boxplot(all_tech_data, output_dir):
    ...
    legend_elements = [
        Line2D(
            [0], [0], color="#333333", linestyle="--", lw=2, label="Base Case (100%)"
        )
    ]
    ...
    for idx, tech_id in enumerate(tech_keys):
        ...
        legend_elements.append(
            Patch(facecolor=t_color, edgecolor="#333333", alpha=0.85, label=tech_label)
        )
        ...
        if valid_box_data:
            ...
            for box in bp["boxes"]:
                box.set(
                    facecolor=t_color, alpha=0.85, linewidth=1.2, edgecolor="#333333"
                )
            for median in bp["medians"]:
    ...
    ax.set_ylabel(
        "Installed Capacity vs. Base Case (%)", fontsize=18, labelpad=15, weight="bold"
    )
    ...
    ax.set_xticks([2025, 2030, 2035, 2040])
    ax.set_xticklabels(
        [2025, 2030, 2035, 2040], rotation=0, ha="center", fontsize=16, weight="bold"
    )
